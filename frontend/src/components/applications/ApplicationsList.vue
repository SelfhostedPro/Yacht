<template lang="html">
  ...
  <div class="namecell">
    <v-menu :close-on-click="true" :close-on-content-click="true" offset-y>
      ...
      <v-list color="foreground" dense>
        <v-list-item @click="AppAction({ Name: item.name, Action: 'start' })">
          <v-list-item-icon>
            <v-icon>mdi-play</v-icon>
          </v-list-item-icon>
          <v-list-item-title>Start</v-list-item-title>
        </v-list-item>
        <v-list-item @click="AppAction({ Name: item.name, Action: 'stop' })">
          <v-list-item-icon>
    ...
    <v-tooltip right v-if="item.isUpdatable" color="primary" class="mb-2">
      <template v-slot:activator="{ on, attrs }">
        <v-avatar
          class="ml-1"
          v-bind="attrs"
  ...
  <div class="projectcell">
    <span>{{ item.Config.Labels["com.docker.compose.project"] || "-" }} </span>
  </div>
</template>
    ...
    <v-tooltip top transition="scale-transition">
      <template v-slot:activator="{ on, attrs }">
        <v-chip
          ...
          >{{
            item.Config.Labels[`local.yacht.port.${port.hport}`] || port.hport
          }}</v-chip
        >
        <v-chip
          v-on="on"
